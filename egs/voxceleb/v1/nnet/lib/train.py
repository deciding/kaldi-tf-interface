...
if __name__ == '__main__':
    ...
    if os.path.isfile(str(learning_rate)):
        ...
        if params.num_epochs > len(learning_rate_array):
            params.num_epochs = len(learning_rate_array) - 1
        ...
        assert float(learning_rate) > 0, "The learning rate should be a number greater than 0 if it is not a file"
        ...
        if not os.path.isfile(str(learning_rate)):
            ...
            if valid_loss < min_valid_loss.min_loss:
                ...
                if epoch - min_valid_loss.min_loss_epoch >= params.reduce_lr_epochs:
                    if epoch >= params.lr_start_decay_epoch:
                        new_learning_rate /= params.learning_rate_reduce_factor
