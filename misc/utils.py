...
def save_phone_class(cont, model, phone_class_filename):
    if cont:
        if not os.path.isdir(os.path.join(model, "nnet")):
            sys.exit("To continue training the model, nnet and codes must be existed in %s." % model)
        phone_class_filename = os.path.join(model, "nnet/phone_class.txt")
    else:
        shutil.copyfile(phone_class_filename, os.path.join(model, "nnet", "phone_class.txt"))

    phone_class = []
    with open(phone_class_filename, "r") as f:
...
class PhoneticDataLoaderParam(object):
    """ A helper class which contains information for data_load_phonetic
    """
    def __init__(self, post_dir, data_dir, spklist,
                 left_context=0, right_context=0,
                 phone_set=None, transid_to_pdfid=None, phones=None,
                 alpha=None, post_map=None, ali_dir=None):
        self.phone_set = phone_set
        self.transid_to_pdfid = transid_to_pdfid
        self.phones = phones
        self.alpha = alpha
...
def compute_kaldi_restricted_attention(endpoints, params):
    ...
    batch_size, length, total_dim = value.shape
    ...
    weights = np.zeros((batch_size, num_heads, length, context_size))
    for i in range(batch_size):
        for j in range(num_heads):
            for k in range(length):
                for l in range(-half_context_size, half_context_size+1):
                    if k + l < 0 or k + l >= length:
                        weights[i, j, k, l + half_context_size] = query[i, j, k, key_dim + l + half_context_size]
                        continue
                    weights[i, j, k, l + half_context_size] = np.dot(query[i, j, k, :key_dim], key[i, j, k + l, :]) + query[i, j, k, key_dim + l + half_context_size]
    weights = softmax(weights, 3)

    attention_output = np.zeros((batch_size, num_heads, length, value_dim))
    for i in range(batch_size):
        for j in range(num_heads):
            for k in range(length):
                temp = np.zeros((value_dim))
                eye = np.eye(context_size)
                for l in range(-half_context_size, half_context_size + 1):
                    if k + l < 0 or k + l >= length:
                        temp += weights[i, j, k, l + half_context_size] * np.concatenate([np.zeros((value.shape[-1],)), eye[l + half_context_size, :]], axis=-1)
                        continue
                    temp += weights[i, j, k, l + half_context_size] * np.concatenate([value[i, j, k+l, :], eye[l + half_context_size, :]], axis=-1)
                attention_output[i, j, k, :] = temp
    attention_output = np.reshape(np.transpose(attention_output, [0, 2, 1, 3]), [batch_size, length, value_dim*num_heads])
