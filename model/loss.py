...
def softmax_weighted(weights, features, labels, num_outputs, params, is_training=None, reuse_variables=None, name="softmax"):
    assert len(shape_list(features)) == len(shape_list(labels)) + 1
    ...
    with tf.variable_scope(name, reuse=reuse_variables):
        ...
        endpoints["logits"] = logits
        endpoints["labels"] = labels
        tf.summary.scalar("%s_loss" % name, loss)
...
def aux_loss_func(features, labels, num_outputs, params, is_training=None, reuse_variables=None, name="auxiliary_loss"):
    ...
    Fix:
        1. Fix bugs in MHE code. (2019/11/11)
           The MHE should be mean(1/||x1-x2||^2) rather than 1/mean(||x1-x2||^2)
    ...
    for loss_func in params.aux_loss_func:
        if loss_func == "ring_loss":
            ...
            # A sophisticated lambda
            mhe_lambda = float(params.mhe_lambda)
            tf.logging.info("MHE lambda is %f" % mhe_lambda)
            if "mhe_start_step" in params.dict:
                mhe_lambda = tf.where(tf.less_equal(params.mhe_start_step, params.global_step),
                                     mhe_lambda, 0)
            if "mhe_type" not in params.dict:
                params.dict["mhe_type"] = 1

            # MHE need to load the softmax weights
            tf.logging.info("MHE is added. MHE type %d" % params.mhe_type)
            with tf.variable_scope(name, reuse=True):
                w = tf.get_variable("output/kernel")
                # Norm w first
                w_norm = tf.nn.l2_normalize(w, dim=0)
                trans_w = tf.transpose(w_norm)
                num_classes = shape_list(trans_w)[0]

                if params.mhe_type == 1:
                    sel_w = tf.gather(trans_w, labels)
                    batch_size = shape_list(sel_w)[0]
                    # Version 1
                    # The inner product may be faster than the element-wise operations.
                    dist = (2.0 - 2.0 * tf.matmul(sel_w, w_norm))
                    mask = tf.scatter_nd(tf.stack([tf.range(batch_size), labels], axis=1),
                                         tf.ones((batch_size)),
                                         shape=(batch_size, num_classes))
                    dist = (1.0 - mask) * dist + mask * 1e6

                    mhe_loss = mhe_lambda * (tf.reduce_sum((1.0 - mask) * (1.0 / dist)) /
                                             tf.to_float(batch_size * (num_classes - 1)))
                    ...
                    dist = 2.0 - 2.0 * tf.matmul(trans_w, w_norm)
                    mask = tf.eye(num_classes)
                    dist = (1.0 - mask) * dist + mask * 1e6
                    mhe_loss = mhe_lambda * (tf.reduce_sum((1.0 - mask) * (1.0 / dist)) /
                                             tf.to_float(num_classes * (num_classes - 1)))
                    # # This is the wrong formula
                    # mhe_loss = mhe_lambda * (1.0 / (tf.reduce_mean(2.0 - 2.0 * tf.matmul(trans_w, w_norm)) + 1e-6))
                else:
                    raise ValueError("Not implemented.")

                loss += mhe_loss
                tf.summary.scalar("mhe_loss", mhe_loss)
                tf.summary.scalar("mhe_lambda", mhe_lambda)
                endpoints["w"] = w
            pass
        else:
...
if __name__ == "__main__":
    ...
    params.dict["mhe_lambda"] = 10
    params.dict["mhe_type"] = 2
    ...
    with tf.Session() as sess:
        ...
        loss_tf, w_val = sess.run([loss, w], feed_dict={embeddings: embeddings_val,
                                            labels: labels_val})
        ...
        loss_np = compute_mhe(labels_val, params, w_val)
        assert np.allclose(loss_tf, loss_np)
        quit()

    params.dict["weight_l2_regularizer"] = 1e-5
    params.dict["arcsoftmax_lambda_min"] = 0
    params.dict["arcsoftmax_lambda_base"] = 0
    params.dict["arcsoftmax_lambda_gamma"] = 1
    params.dict["arcsoftmax_lambda_power"] = 1
    params.dict["arcsoftmax_m"] = 0.3
    params.dict["global_step"] = 1000
    params.dict["feature_norm"] = False
    params.dict["aux_loss_func"] = []

    loss, endpoints = additive_angular_margin_softmax(embeddings, labels, num_labels, params, is_training=True, reuse_variables=tf.AUTO_REUSE)
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        loss_tf, w_val = sess.run([loss, params.softmax_w], feed_dict={embeddings: embeddings_val,
                                                                 labels: labels_val})
